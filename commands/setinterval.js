...
module.exports = {
  name: 'setinterval',
  description: `Sets an interval for status messages to be sent in.`,
  ...
  async execute(message, args, prefix) {
    ...
    if (!args[1] || isNaN(args[1])) {
      let msg = await message.channel.send(`Proper command usage ${prefix}setinterval [channel-name] [minutes]`);
    ...
    /* if (args[1] < 5) {
      let msg = await message.channel.send(`Minutes can't be lower than 5.`);
      msg.delete({timeout: deletionTimeout });
      return message.react(reactionError);
    } */
    ...
    if (!server) {
      let msg = await message.channel.send(`This server doesn't have a server linked to it yet. Type ${prefix}setguildserver to setup one.`);
      msg.delete({ timeout: deletionTimeout });
      return message.react(reactionError);
    }
